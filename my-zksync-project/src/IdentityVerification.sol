...
contract IdentityVerification is Ownable {
    ...
    event DocumentHashRegistered(bytes32 indexed documentHash, address indexed owner);
    ...
    // ============================
    function uploadAndRegisterDocument(string memory _docType, bytes32 _docHash) public {
    ...
    // Ownership Check
    // ============================
    function verifyHash(bytes32 documentHash, address claimedOwner) external view returns (bool) {
        return documentOwners[documentHash] == claimedOwner;
    }

    function getHashOwner(bytes32 documentHash) external view returns (address) {
